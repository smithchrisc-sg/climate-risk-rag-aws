...
def fix_import_issue():
    """Fix the import issue by importing classes directly"""
...
def execute_phase1_search(query: str, filters: Dict[str, Any], parameters: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    try:
        # Import classes directly from utils module
        from utils.ContextualEntityAligner import ContextualEntityAligner
        from utils.KnowledgeGraphManager import KnowledgeGraphManager
        
        print("✅ Successfully imported Phase 1 classes directly")
        ...
        # Create mock entities by chunk format
        entities_by_chunk = []
        ...
        document_metadata = {
            'document_id': 'test-doc-1',
            'title': 'Test Document for North Macedonia',
            'source': 'api_test'
        }
        ...
        print(f"✅ Phase 1 alignment completed: {len(aligned_results)} results")
        
        # Build search results
        results = []
        for alignment in aligned_results:
            results.append({
                'document_id': f'phase1-{alignment.get("entity_text", "unknown").replace(" ", "-").lower()}',
                'title': f'Phase 1 Entity Alignment: {alignment.get("entity_text", "Unknown")}',
                'content': f'Entity: {alignment.get("entity_text", "")} aligned to {alignment.get("aligned_uri", "")} with confidence {alignment.get("confidence_score", 0):.3f}',
                'score': alignment.get('confidence_score', 0.8),
                'source': 'phase1_entity_alignment',
                'entity_data': alignment
            })
        
        # Add general result if no specific alignments
        if not results:
            results.append({
                'document_id': 'phase1-connected',
                'title': f'Phase 1 Infrastructure Connected: {query}',
                'content': f'Phase 1 successfully connected and processed {len(entities_by_chunk)} chunks. No specific entity alignments found.',
                'score': 0.7,
...
def get_diagnostic_response(query: str, error: str) -> Dict[str, Any]:
    ...
    return {
        'results': [{
            ...
            'summary': f'Phase 1 connection attempt failed. Error: {error[:150]}...',
            'score': 0.6,
...
'''
    ...
    with zipfile.ZipFile('lambda_import_fix.zip', 'w') as zf:
        zf.writestr('lambda_function.py', code)
    
    try:
        with open('lambda_import_fix.zip', 'rb') as f:
        ...
        if os.path.exists('lambda_import_fix.zip'):
            os.remove('lambda_import_fix.zip')
