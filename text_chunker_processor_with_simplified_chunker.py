...
class TextChunkerProcessor:
    def __init__(self):
        ...
        # Import simplified smart chunker from shared layer
        try:
            from simplified_smart_chunker import SimplifiedSmartChunker
            
            # Initialize simplified smart chunker with optimized settings
            self.structured_chunker = SimplifiedSmartChunker(
                sentences_per_chunk=5,      # Target 5 sentences per chunk
            ...
            logger.info("Using simplified smart chunker with sentence-based overlap")
        except ImportError as e:
            logger.warning(f"Simplified smart chunker not available: {e}")
    ...
    def create_chunks(self, full_text: str, textract_structure: Optional[Dict], doc_id: str) -> List[Dict]:
        ...
        try:
            if self.structured_chunker and textract_structure:
                # Use simplified smart chunker with Textract data
                logger.info("Using simplified smart chunker with Textract structure")
                try:
                    chunks = self.structured_chunker.chunk_document(textract_structure)
                    logger.info(f"Successfully created {len(chunks)} chunks using simplified smart chunker with Textract")
                except Exception as e:
                    logger.error(f"Error in simplified smart chunking: {e}")
                    logger.warning("Falling back to text-only chunking")
                    chunks = self.structured_chunker.chunk_text(full_text)
            elif self.structured_chunker:
                # Use simplified smart chunker without structure
                logger.info("Using simplified smart chunker (text-only)")
                try:
                    chunks = self.structured_chunker.chunk_text(full_text)
                    logger.info(f"Successfully created {len(chunks)} chunks using simplified smart chunker (text-only)")
                except Exception as e:
                    logger.error(f"Error in simplified smart text-only chunking: {e}")
    ...
    def basic_chunk_text(self, text: str, chunk_size: int = 1000, overlap: int = 200) -> List[Dict]:
