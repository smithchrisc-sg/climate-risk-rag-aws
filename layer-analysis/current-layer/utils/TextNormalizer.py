...
# Optional NLTK imports for enhanced functionality
try:
    import nltk
...
class TextNormalizer:
    ...
    def __init__(self, download_nltk_data: bool = True):
        """
        Initialize text normalizer with optional NLTK components
        
        Args:
            download_nltk_data: Whether to download required NLTK data (ignored if NLTK not available)
        """
        ...
        if NLTK_AVAILABLE:
            ...
            try:
                ...
                self.stopwords = self._get_minimal_stopwords()
        else:
            self.logger.info("NLTK not available, using fallback implementations")
            self.stemmer = None
        ...
        self.logger.debug(f"TextNormalizer initialized (NLTK available: {NLTK_AVAILABLE})")
    
    def _tokenize(self, text: str) -> List[str]:
        """Tokenize text with fallback when NLTK is not available"""
        if NLTK_AVAILABLE:
            try:
    ...
    def _get_minimal_stopwords(self) -> Set[str]:
        """Get minimal set of stopwords when NLTK is not available"""
        return {
    ...
    def _stem_text(self, text: str) -> str:
        ...
        if not NLTK_AVAILABLE or self.stemmer is None:
            # Fallback: simple suffix removal
            return self._simple_stem(basic)
        
        try:
            tokens = self._tokenize(basic)
