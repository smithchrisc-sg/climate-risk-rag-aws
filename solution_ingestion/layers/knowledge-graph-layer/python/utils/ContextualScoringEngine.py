...
class ContextualScoringEngine:
    ...
    def score_alignments_with_context(self, alignments: List[Any], entity_text: str, entity_type: str,
        ...
        for alignment in alignments:
            self.logger.info(f"ContextualScoringEngine: Scoring alignment for {alignment.entity_text}")
            self.logger.info(f"ContextualScoringEngine: Alignment: {alignment}")
            self.logger.info(f"ContextualScoringEngine: Document metadata: {document_metadata}")
            self.logger.info(f"ContextualScoringEngine: Chunk context: {chunk_context}")
            ...
            original_score = alignment.confidence_score
            ...
            self.logger.debug(f"Entity '{alignment.entity_text}' score: {original_score:.3f} -> {alignment.confidence_score:.3f} (boost: +{context_boost:.3f})")
    ...
    def _score_geographic_context(self, alignment: Any, document_metadata: Dict[str, Any], 
        ...
        self.logger.info(f"ContextualScoringEngine: Scoring geographic entity alignment for {alignment.entity_text}")
        self.logger.info(f"ContextualScoringEngine: Document metadata: {document_metadata}")
        self.logger.info(f"ContextualScoringEngine: Chunk context: {chunk_context}")
        ...
        if any(signal in doc_title for signal in ['north american', 'usa', 'united states', 'canada']):
            if metadata.get('country') in ['US', 'CA']:
                ...
                self.logger.info(f"Applied North American context boost: +{self.weight_document_title}")
            else:
        ...
        # European context boost
            ...
            if metadata.get('country') in eu_countries:
                boost += self.weight_document_title
                self.logger.info(f"Applied European context boost: +{self.weight_document_title}")
        ...
        # Asian context boost
            ...
            if metadata.get('country') in asian_countries:
                boost += self.weight_document_title
                self.logger.info(f"Applied Asian context boost: +{self.weight_document_title}")
        ...
        if admin1 and any(admin1.lower() in loc.lower() for loc in chunk_locations):
            ...
            self.logger.info(f"Applied state/province co-occurrence boost: +{self.weight_chunk_cooccurrence}")
        
        # Country co-occurrence
        self.logger.info(f"ContextualScoringEngine: Metadata: {metadata}")
        country = metadata.get('country', '')
        self.logger.info(f"ContextualScoringEngine: Country: {country}")    
        if country is not None:
            ...
            if any(country_name.lower() in ' '.join(chunk_locations).lower() for country_name in country_names):
                ...
                self.logger.info(f"Applied country co-occurrence boost: +{self.weight_chunk_cooccurrence * 0.8}")
        ...
        if population:
            try:
                ...
                if population_int > 1000000:  # Major city
                    boost += self.weight_entity_type_match
                elif population_int > 100000:  # Medium city
    ...
    def _identify_context_signals(self, alignment: Any, document_metadata: Dict[str, Any],
        ...
        if population:
            try:
                population_int = int(population)
                if population_int > 1000000:
                    signals.append('major_city_population')
                elif population_int > 100000:
    ...
    def _get_country_names(self, country_code: str) -> List[str]:
        ...
        self.logger.info(f"ContextualScoringEngine: Getting country names for {country_code}")
        if country_code is None:
            self.logger.info(f"ContextualScoringEngine: Country code is None")
            return []
