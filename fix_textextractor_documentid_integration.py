#!/usr/bin/env python3
"""
Fix TextExtractor to properly integrate with DocumentIDManager
This script creates the corrected TextExtractor Processor code
"""

import os

def create_fixed_textextractor_processor():
    """Create the corrected TextExtractor Processor that uses DocumentIDManager"""
...
def main():
    """Create the fixed TextExtractor Processor"""
    print("🔧 Creating Fixed TextExtractor Processor")
    print("=" * 50)
    
    # Create the fixed code
    fixed_code = create_fixed_textextractor_processor()
    
    # Save to a new file for review
    output_file = '/Users/chris/climate-risk-rag-aws/lambda/text_extractor_processor/text_extractor_processor_FIXED.py'
    
    with open(output_file, 'w') as f:
        f.write(fixed_code)
    
    print(f"✅ Created fixed TextExtractor Processor: {output_file}")
    print()
    print("🔧 Key Fixes Applied:")
    print("  • Integrated DocumentIDManager for proper GUID-based doc_id")
    print("  • Maintains doc_hash for TextExtractor system compatibility")
    print("  • Updates DocumentIDManager with processing status")
    print("  • Uses proper doc_id for directory structure")
    print("  • Maintains backward compatibility with existing system")
    print()
    print("📋 Integration Points:")
    print("  • doc_id: GUID from DocumentIDManager (for directory structure)")
    print("  • doc_hash: Content hash from TextExtractor (for job tracking)")
    print("  • Both systems work together seamlessly")
    print()
    print("🚨 IMPORTANT:")
    print("  • Review the fixed code before deployment")
    print("  • Ensure DocumentIDManager is available in Lambda layer")
    print("  • Test with small document first")
