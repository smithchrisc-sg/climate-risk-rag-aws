#!/usr/bin/env python3
"""
English Document NLP Testing Script
Tests NLP processing on a single English climate document for entity analysis
"""
import json
import boto3
import time
...
class EnglishNLPTester:
    """Test NLP processing with English climate documents"""
    ...
    def __init__(self, profile_name='solve-global', region='us-east-1'):
        ...
        logger.info("Initialized English NLP Tester for region {}".format(region))
    ...
    def create_mock_chunks(self, doc_id, full_text):
        ...
        for i, paragraph in enumerate(paragraphs[:15]):  # Limit to 15 chunks for testing
            chunk = {
    ...
    def download_and_analyze_results(self, doc_id):
        ...
        try:
            ...
            for obj in response['Contents']:
                ...
                local_filename = "english_{}".format(file_name)
    ...
    def analyze_entity_categories(self, results):
        ...
        if not results or 'entities_' not in str(results.keys()):
            logger.error("No entities file found in results")
            return
        ...
        # Print analysis
        logger.info("\n" + "="*60)
        logger.info("ENTITY CATEGORY ANALYSIS")
        logger.info("="*60)
        
        for category, entities in sorted(entity_categories.items()):
            logger.info("\n{} ({} entities):".format(category, len(entities)))
            logger.info("-" * 40)
            for entity in sorted(entities, key=lambda x: x['confidence'], reverse=True)[:10]:
                logger.info("  • \"{}\" (confidence: {:.3f})".format(entity['text'], entity['confidence']))
            if len(entities) > 10:
                logger.info("  ... and {} more".format(len(entities) - 10))
        
        return entity_categories
    
    def test_english_document(self, doc_key):
        """Test NLP processing on a single English document"""
        
        doc_id = doc_key.split('/')[-1].replace('.txt', '')
        
        logger.info("\n" + "="*60)
        logger.info("🧪 TESTING ENGLISH DOCUMENT: {}".format(doc_id))
        ...
        try:
            ...
            logger.error("Error in English document test: {}".format(str(e)))
...
def main():
    ...
    # Initialize tester
    tester = EnglishNLPTester()
    
    # Test the English Somalia document
    doc_key = "extracted_text/002328de_230b4003.txt"
    
    logger.info("🚀 STARTING ENGLISH NLP TESTING")
    logger.info("Target document: {}".format(doc_key))
    
    # Run test
    result = tester.test_english_document(doc_key)
    
    # Save results
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    results_file = 'english_nlp_test_results_{}.json'.format(timestamp)
