...
class ElasticSearchSparqlHandler(ElasticSearchBaseHandler):
    ...
    def filter_records(self, records, client):
        """
        Filters records to be stored in OpenSearch using ontology-specific filtering.
        
        Enhanced filtering supports multiple ontologies:
        - Geonames: name-related predicates for location alignment
        - Climate Risk: label predicates for concept alignment
        ...
        4) Ontology-specific filtering rejects the record
        5) Object type is present in excluded_types list
        6) Object if of type lang literal and lang fails regex check
        7) Object if of type Float/ Double literal and value is not finite i.e. NaN, INF, -INF
        8) Property value invalid for property type specified for record
        9) Object is any literal and its value cannot be converted to appropriate ES type.
        ...
        for record in records:
            ...
            if isinstance(statement_elements[SUBJECT], BNode):
                ...
                # Enhanced ontology-specific filtering
                if not self._should_index_record(record_data, statement_elements, obj_key):
                    continue

                if obj_datatype_token in excluded_types:
                    # case 5) Object type is present in excluded_types list
                    logger.debug(
                ...
                if obj_datatype_token == DataType.STRING.value and statement_elements[OBJECT].language:
                    ...
                    if not validate_language(obj_lang):
                        # case 6) Object if of type lang literal and lang fails regex check
                        logger.debug(
                ...
                if obj_datatype_token in {DataType.FLOAT.value, DataType.DOUBLE.value, DataType.DECIMAL.value}:
                    ...
                    if is_valid_float_value(obj_value) and (math.isinf(float(obj_value)) or math.isnan(float(obj_value))):
                        # case 7) Object if of type Float/ Double / Decimal  literal and value is not finite
                        # i.e. NaN, INF, -INF
                        logger.debug(
                ...
                if not field_mapping_type_in_es:
                    ...
                    try:
                        ...
                        if validate(obj_value, field_mapping_type_in_es):
                            ...
                            # case 8) Property value invalid for property type specified for record
                            logger.debug(
                        ...
                        if e.error == "illegal_argument_exception":
                            # case 9) Object is any literal and its value cannot be converted to appropriate ES type.
    ...
    def _should_index_record(self, record_data, statement_elements, predicate_uri):
        """
        Orchestrates ontology-specific filtering decisions for concept alignment.
        
        Determines which ontology the record belongs to and applies appropriate filtering:
        - Climate Risk Ontology: Only rdfs:label predicates for concepts (not properties)
        - Geonames Ontology: Only name-related predicates for locations
        
        :param record_data: Stream record data for logging
        :param statement_elements: Parsed SPARQL statement elements
        :param predicate_uri: The predicate URI to evaluate
        :return: True if record should be indexed, False otherwise
        """
        # Determine ontology based on named graph
        graph_uri = str(statement_elements[GRAPH]) if GRAPH in statement_elements else None
        
        if graph_uri == "http://solve.global/knowledge-commons/climate-risk-ontology":
            return self._filter_climate_risk_records(record_data, statement_elements, predicate_uri)
        else:
            return self._filter_geonames_records(record_data, statement_elements, predicate_uri)

    def _filter_geonames_records(self, record_data, statement_elements, predicate_uri):
        """
        Filter geonames ontology records - only name-related predicates for location alignment.
        
        Supports fuzzy matching for location concept alignment by indexing various name predicates
        that NLP systems might extract (official names, alternate names, etc.).
        ...
        :return: True if record should be indexed, False otherwise
        """
        # Feature flag for safety - allows disabling geonames filtering if needed
        if not self._is_geonames_filtering_enabled():
            logger.debug("Geonames filtering disabled - skipping record {}"
                         .format(str(record_data)))
            return False
        ...
        if predicate_uri.strip() not in name_predicates:
            logger.debug("Dropping Record : Geonames ontology - predicate not in name-related filter for record {}"
                         .format(str(record_data)))
            return False
            
        return True

    def _filter_climate_risk_records(self, record_data, statement_elements, predicate_uri):
        """
        Filter climate risk ontology records - only label predicates for concept alignment.
        
        Supports fuzzy matching for climate risk concept alignment by indexing label predicates
        for concepts (excluding properties) that NLP systems might extract from documents.
        ...
        :return: True if record should be indexed, False otherwise
        """
        # Feature flag for safety - allows enabling/disabling climate risk filtering
        if not self._is_climate_risk_filtering_enabled():
            logger.debug("Dropping Record : Climate risk filtering disabled - skipping record {}"
                         .format(str(record_data)))
            return False
        
        # Only process label-related predicates for concept alignment
        if not self._is_climate_risk_label_predicate(predicate_uri.strip()):
            logger.debug("Dropping Record : Climate risk ontology - not a label predicate for record {}"
                         .format(str(record_data)))
            return False
        
        # Skip property subjects (only index concepts for NLP alignment)
        subject_uri = str(statement_elements[SUBJECT])
        if self._is_climate_risk_property(subject_uri):
            logger.debug("Dropping Record : Climate risk ontology - property subject excluded for record {}"
                         .format(str(record_data)))
            return False
            
        return True

    def _is_climate_risk_label_predicate(self, predicate_uri):
        """
        Check if predicate is a label-related predicate suitable for concept alignment.
        
        Includes current and future label predicates to support fuzzy matching
        and multi-language concept alignment.
        
        :param predicate_uri: The predicate URI to check
        :return: True if it's a label predicate, False otherwise
        """
        climate_risk_labels = {
            "http://www.w3.org/2000/01/rdf-schema#label",           # Current: rdfs:label
            "http://www.w3.org/2004/02/skos/core#prefLabel",        # Future: skos:prefLabel  
            "http://www.w3.org/2004/02/skos/core#altLabel",         # Future: skos:altLabel
            "http://purl.org/dc/elements/1.1/title",                # Future: dc:title
            "http://purl.org/dc/terms/title"                        # Future: dcterms:title
        }
        return predicate_uri in climate_risk_labels

    def _is_climate_risk_property(self, subject_uri):
        """
        Pattern-based detection of climate risk ontology properties.
        
        Uses naming convention patterns to identify properties vs concepts,
        providing flexibility as the ontology evolves without requiring code changes
        for each new property.
        
        :param subject_uri: The subject URI to check
        :return: True if it's a property, False if it's a concept
        """
        if not subject_uri.startswith("http://solve.global/knowledge-commons/climate-risk-ontology#"):
            return False
        
        local_name = subject_uri.split("#")[-1]
        
        # Property naming patterns based on OWL conventions
        # These patterns cover most property types and adapt to new properties automatically
        property_patterns = [
            "has",           # hasCharacteristic, hasComponent, hasCobenefits, etc.
            "is",            # isLocatedIn, isVulnerableTo, etc.  
            "provides",      # providesService, providesExpertise
            "requires",      # requiresStakeholderEngagement
            "supports",      # supportsAdaptation
            "implements",    # implementsStrategy, implementsAdaptationStrategy
            "addresses",     # addressesRisk
            "mitigates",     # mitigates, mitigatesImpact, mitigatesClimateImpact
            "enhances",      # enhancesResilience
            "reduces",       # reducesEmissions
            "causes",        # causesImpact
            "affects",       # affectedBy (property would be "affects")
            "contains",      # containsFeature, containsWaterBody
            "experiences",   # experiencesClimateEvent
            "manifests",     # manifestsAs
            "drives",        # drivesRisk
            "exacerbates",   # exacerbatesEvent, exacerbatesRisk
            "intensifies",   # intensifies
            "precedes",      # precedes
            "operates",      # operatesIn
            "collaborates",  # collaboratesWith
            "involves",      # involvesStakeholder
            "impacts",       # impactsSystem
            "prevents",      # preventsOrReduces
            "directly",      # directlyContributesTo
            "indirectly",    # indirectlyInfluences
            "adjacent"       # adjacentTo
        ]
        
        return any(local_name.startswith(pattern) for pattern in property_patterns)

    def _is_geonames_filtering_enabled(self):
        """
        Feature flag for geonames filtering.
        
        Enabled by default for backward compatibility with existing working system.
        Can be disabled for testing or troubleshooting.
        
        :return: True if geonames filtering is enabled
        """
        return os.environ.get('ENABLE_GEONAMES_FILTERING', 'true').lower() == 'true'

    def _is_climate_risk_filtering_enabled(self):
        """
        Feature flag for climate risk filtering.
        
        Disabled by default for safety - must be explicitly enabled.
        This ensures existing geonames functionality continues working unchanged.
        
        :return: True if climate risk filtering is enabled
        """
        return os.environ.get('ENABLE_CLIMATE_RISK_FILTERING', 'false').lower() == 'true'
