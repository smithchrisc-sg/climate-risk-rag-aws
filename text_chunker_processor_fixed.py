...
class TextChunkerProcessor:
    def __init__(self):
        ...
        try:
            ...
            # Add our own safe sentence splitting method
            self._add_safe_sentence_splitting()
    ...
    def _add_safe_sentence_splitting(self):
        """Add safe sentence splitting method to the structured chunker"""
        if not self.structured_chunker:
            return
        
        # Define a safe sentence splitting method that properly handles 'self'
        def _split_into_sentences_safe(self_chunker, text: str) -> List[str]:
            ...
            if len(sentences) == 1 and len(text) > self_chunker.max_chunk_size:
        ...
        # Replace the method in the structured chunker using types.MethodType to properly bind it
        self.structured_chunker._split_into_sentences = types.MethodType(_split_into_sentences_safe, self.structured_chunker)
        
        # Log success
        logger.info("Successfully added safe sentence splitting method to SmartStructuredChunker")
        ...
        try:
            if self.structured_chunker and textract_structure:
                ...
                try:
                    ...
                    logger.info(f"Successfully created {len(chunks)} chunks using smart structured chunking with Textract")
                except Exception as e:
                ...
                try:
                    ...
                    logger.info(f"Successfully created {len(chunks)} chunks using smart structured chunking (text-only)")
                except Exception as e:
