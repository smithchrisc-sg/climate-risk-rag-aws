    ...
    def store_chunks_to_s3(self, chunks: List[Dict], doc_id: str, original_message: Dict, full_text: str) -> List[str]:
        ...
        try:
            ...
            metadata = {
                ...
                'version': 'v2',
                'total_chunks': len(chunks),
                'chunking_strategy': 'smart_overlap' if self.structured_chunker else 'basic_v2',
                'original_text_length': len(full_text),
                'processing_timestamp': datetime.utcnow().isoformat(),
                'original_message': original_message,
                'implementation_notes': 'New v2 chunking implementation - separate from migrated data',
                'chunk_summary': {
                    'chunk_types': list(set(chunk.get('chunk_type', 'text') for chunk in chunks)),
                    'avg_chunk_size': sum(len(chunk['text']) for chunk in chunks) / len(chunks) if chunks else 0,
                    'total_text_length': sum(len(chunk['text']) for chunk in chunks)
            ...
            # Store full text for NLP processing
            full_text_key = f"{doc_prefix}full_text.json"
            full_text_data = {
                'doc_id': doc_id,
                'version': 'v2',
                'full_text': full_text,
                'text_length': len(full_text),
                'created_at': datetime.utcnow().isoformat(),
                'purpose': 'nlp_processing',
                'implementation': 'v2_chunking'
            }
            ...
            summary_key = f"{doc_prefix}summary.json"
            summary = {
                'doc_id': doc_id,
                'version': 'v2',
                'processing_complete': True,
                'chunks_created': len(chunks),
                'files_created': len(s3_keys),
                's3_keys': s3_keys,
                'processing_timestamp': datetime.utcnow().isoformat(),
                'chunking_method': 'smart_overlap' if self.structured_chunker else 'basic_v2',
                'separation_note': 'Stored in v2-chunks to separate from migrated data'
            }
            ...
            s3_keys.append(summary_key)
            
            logger.info(f"Stored {len(chunks)} chunks and metadata for document {doc_id} in v2-chunks structure")
            return s3_keys
