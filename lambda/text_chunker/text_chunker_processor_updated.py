...
class TextChunkerProcessor:
    ...
    def process_sqs_record(self, record: Dict) -> Dict:
        ...
        try:
            ...
            if not self.message_parser.validate_message_format(message, 'text_ready'):
                raise ValueError("Invalid message format or stage")
    ...
    def process_text_chunking(self, processing_info: Dict) -> Dict:
        ...
        structure_location = processing_info.get('structure_location')
        ...
        try:
            ...
            self.publish_chunks_ready(doc_id, doc_hash, chunks_location, text_location, 
    ...
    def save_chunks_to_s3(self, doc_id: str, chunks: List[Dict], full_text: str) -> str:
        ...
        try:
            base_path = f"chunks/{doc_id}"
            
            # Save individual chunk files
            for chunk in chunks:
                chunk_key = f"{base_path}/{chunk['chunk_id']}.json"
            ...
            summary = {
                ...
                'total_characters': len(full_text),
                'created_at': datetime.utcnow().isoformat() + 'Z',
                'chunk_files': [f"{chunk['chunk_id']}.json" for chunk in chunks]
            }
