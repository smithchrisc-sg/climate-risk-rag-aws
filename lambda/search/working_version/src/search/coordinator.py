...
class SearchCoordinator:
    ...
    async def search(self, query: str, filters: Dict[str, Any], 
        ...
        try:
            ...
            # Log score ranges for each method
            if keyword_results.results:
                kw_scores = [r.score for r in keyword_results.results]
                kw_docs = [r.document_id for r in keyword_results.results[:3]]
                self.logger.info(f"[{search_id}] Keyword scores: {min(kw_scores):.4f}-{max(kw_scores):.4f} (avg: {sum(kw_scores)/len(kw_scores):.4f})")
                self.logger.info(f"[{search_id}] Keyword top docs: {kw_docs}")
            
            if vector_results.results:
                vec_scores = [r.score for r in vector_results.results]
                vec_docs = [r.document_id for r in vector_results.results[:3]]
                self.logger.info(f"[{search_id}] Vector scores: {min(vec_scores):.4f}-{max(vec_scores):.4f} (avg: {sum(vec_scores)/len(vec_scores):.4f})")
                self.logger.info(f"[{search_id}] Vector top docs: {vec_docs}")
                
                # Check for overlap
                kw_doc_set = set(r.document_id for r in keyword_results.results)
                vec_doc_set = set(r.document_id for r in vector_results.results)
                overlap = kw_doc_set.intersection(vec_doc_set)
                self.logger.info(f"[{search_id}] Document overlap: {len(overlap)} docs ({list(overlap)[:3]}...)")
            
            if graph_results.results:
                graph_scores = [r.score for r in graph_results.results]
                self.logger.info(f"[{search_id}] Graph scores: {min(graph_scores):.4f}-{max(graph_scores):.4f} (avg: {sum(graph_scores)/len(graph_scores):.4f})")
            
            # Log any errors in search responses
            for method, results in [("keyword", keyword_results), ("vector", vector_results), ("graph", graph_results)]:
                if hasattr(results, 'metadata') and results.metadata.get('error'):
                    self.logger.error(f"[{search_id}] {method} search error: {results.metadata['error']}")
