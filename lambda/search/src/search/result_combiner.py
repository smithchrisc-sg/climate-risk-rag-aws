...
class ResultCombiner:
    ...
    def combine_results(self,
        ...
        try:
            ...
            # Collect all results with source marking
            all_results = []
            
            # Add keyword results with source marking
            for result in normalized_results[ResultType.KEYWORD].results:
                result.source = ResultType.KEYWORD
                all_results.append(result)
                
            # Add vector results with source marking
            for result in normalized_results[ResultType.VECTOR].results:
                result.source = ResultType.VECTOR
                all_results.append(result)
                
            # Add graph results with source marking
            for result in normalized_results[ResultType.GRAPH].results:
                result.source = ResultType.GRAPH
                all_results.append(result)

            # Score and rank all results
            scored_results = self._score_and_rank_results(all_results)
    ...
    def merge_to_single_response(self, normalized_results: Dict[ResultType, SearchResponse]) -> SearchResponse:
        ...
        try:
            ...
            # Deduplicate by document_id and merge search types
            document_map = {}
            for result in all_results:
                ...
                if doc_id in document_map:
                    ...
                    if result.score > existing.score:
                        ...
                        existing.content = result.content  # Use content from higher scoring result
