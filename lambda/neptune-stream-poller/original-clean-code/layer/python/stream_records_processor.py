...
def __get_query_language__(stream_endpoint):
    ...
    """
    def is_valid_stream_endpoint(token):
        return stream_endpoint.lower().endswith("{}/{}".format(token.lower(), "stream"))

    for token in (GREMLIN, SPARQL, PROPERTY_GRAPH, PROPERTY_GRAPH_ALIAS):
        if is_valid_stream_endpoint(token):
            return token.lower()
...
class StreamRecordsProcessor:
    ...
    def write(self, query_queue):

        """
        Read batched queries from query_queue and pass these queries to the configured Handler for writing to Neptune/ES.

        Handler for writing stream records can be set in ConfigProvider using parameter "stream_records_handler_name".

        If there are no more queries in query_queue, response form handler will be None else appropriate response
        from handler is returned

        :param query_queue: Thread safe queue implementation
    ...
    def write_with_metrics(self, query_queue, lease, lease_manager, metrics_publisher_client):
        ...
        If there are no more queries in query_queue, response form handler will be None and False will be returned to
        stop continuous write

        Else response from handler is used to update DDB lease table and publish metrics

        :param query_queue: Thread safe queue implementation
        :return: True if there is response from handler, else False
        """

        result = self.write(query_queue)

        if result is None:
            # No records in Queue to write
            ...
            logger.debug("No more records in queue to process.")
            return False  # Stop Continuous Write from Stream and wait for some time
        ...
        metrics_publisher_client.publish_metrics([metrics_publisher_client.generate_record_processed_metrics(result.records_processed)])
        logger.info("Finished publishing data to Metrics")

        # Publish Lag Metrics
        metrics_publisher_client.publish_metrics([metrics_publisher_client.generate_stream_lag_metrics
                                                  (self.__get_stream_lag_time(result.last_transaction_time_stamp))])

        logger.info("Finished writing Stream records. Last Processed event id (commitNum, OpNum) - {} , {}"
                    .format(result.last_commit_num, result.last_op_num))

        return True  # No wait required when records are found

    def fetch_stream_records(self, limit, commit_num, op_num, query_queue):
        ...
        Handler for processing stream records can be set in ConfigProvider using parameter "stream_records_handler_name".
        ...
        logger.debug("Start processing Stream Records...")
        # Calling Handler to further process Stream Records
        return stream_records_handler.handle_records(stream_log, query_queue)
    ...
    def process(self, limit, commit_num, op_num):
        ...
        This method is added to make code backward compatible
