...
class TextExtractorProcessor:
    ...
    def publish_text_extraction_complete(self, doc_id: str, doc_hash: str, job_metadata: Dict, processing_metadata: Dict):
        ...
        try:
            # Construct S3 locations
            text_location = f"s3://{self.output_bucket}/extracted_documents/{doc_hash}/raw_text.txt"
            structure_location = f"s3://{self.output_bucket}/extracted_documents/{doc_hash}/textract_response.json"
    ...
    def process_successful_job(self, job_id: str, job_metadata: Dict) -> Dict:
        ...
        try:
            ...
            doc_hash = job_metadata['doc_hash']
            logger.info(f"Saving structured output for doc_hash: {doc_hash}")
            
            files_created = self.save_structured_output(doc_hash, textract_response, job_metadata)
            ...
            logger.info(f"Saved {len(files_created)} files for doc_hash: {doc_hash}")
    ...
    def save_structured_output(self, doc_hash: str, textract_response: Dict, job_metadata: Dict) -> List[str]:
        """Save structured Textract output to S3"""
        
        files_created = []
        base_path = f"extracted_documents/{doc_hash}"
    ...
    def get_or_create_doc_id(self, job_metadata: Dict) -> str:
        """Get or create document ID using DocumentIDManager"""
        try:
            if self.doc_id_manager:
                bucket = job_metadata['source_bucket']
                key = job_metadata['source_key']
                return self.doc_id_manager.get_or_create_id_from_s3(bucket, key)
            else:
                # Fallback to doc_hash
                return job_metadata['doc_hash']
        except Exception as e:
            logger.error(f"Error getting doc_id: {e}")
            return job_metadata['doc_hash']
