    ...
    def build_pattern_automaton(self, ontology_domains: List[str] = None, force_rebuild: bool = False) -> bool:
        ...
        try:
            ...
            if terms_to_add:
                ...
                self.logger.info(f"Pattern automaton built: {self.stats['terms_indexed']} terms, "
            ...
            for match in matches:
                ...
                for candidate in concept_candidates:
                    ...
                    confidence, factors = self.confidence_scorer.calculate_alignment_confidence(
                        ...
                        entity_metadata={
                            'chunk_id': chunk_id,
                            'position': start_pos / len(text) if text else 0.0,
                            'section_type': chunk_metadata.get('section_type', 'unknown')
                        }
                    )
                    
                    if confidence >= self.min_confidence:
                        # Generate URI for this mention using EXISTING uri_manager
                        mention_uri = self._mint_concept_mention_uri(
                            chunk_id,
                        ...
                        mention = {
                            ...
                            'section_type': chunk_metadata.get('section_type', 'unknown'),
                            'hierarchy_level': chunk_metadata.get('hierarchy_level', 0),
                            'parent_chunk_id': chunk_metadata.get('parent_chunk_id'),
                            'detection_method': 'aho_corasick_pattern'
                        }
                        
                        mentions.append(mention)
        
        except Exception as e:
            self.logger.warning(f"Pattern matching failed for chunk {chunk_id}: {e}")
        
        # Remove duplicate mentions (same concept, overlapping positions)
