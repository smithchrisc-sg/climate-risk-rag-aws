...
class StructureAwareProcessor:
    ...
    def prepare_structure_enhanced_document(self, doc_id: str, full_text: str, 
        ...
        try:
            ...
            if textract_structure and self._has_valid_structure(textract_structure):
                ...
                return self._prepare_with_structure(doc_id, full_text, textract_structure, metadata, doc_id)
            else:
                logger.info(f"Falling back to standard processing for {doc_id}")
                return self._prepare_standard_document(doc_id, full_text, metadata, doc_id)
    ...
    def _prepare_with_structure(self, doc_id: str, full_text: str, 
                               textract_structure: Dict, metadata: Dict, doc_id_param: str) -> Dict:
        ...
        doc_data = {
            ...
            "title": self._extract_enhanced_title(structure_elements, metadata, doc_id),
    ...
    def _prepare_standard_document(self, doc_id: str, full_text: str, 
                                  metadata: Dict, doc_id_param: str) -> Dict:
        """Fallback to standard document preparation without structure"""
        
        doc_data = {
            "doc_id": doc_id,
            "content": full_text,
            "title": self._extract_metadata_value(metadata, 'title', self._get_filename_from_url(doc_id)),
    ...
    def _extract_enhanced_title(self, structure_elements: Dict, metadata: Dict, doc_id: str) -> str:
        ...
        if headings:
            ...
            if sorted_headings:
                return sorted_headings[0].get('text', self._get_filename_from_url(doc_id))
        
        return self._get_filename_from_url(doc_id)
    ...
    def _get_filename_from_url(self, doc_id: str) -> str:
        """Get filename by parsing URL from DatabaseManager.get_document_metadata"""
        try:
            # Import DatabaseManager (should be available in the layer)
            ...
            if doc_metadata and doc_metadata.get('url'):
            ...
            logger.warning(f"Could not retrieve filename from URL for {doc_id}: {e}")
            return 'Untitled'
