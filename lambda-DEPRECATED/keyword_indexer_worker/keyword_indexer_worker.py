...
Fixed StructureAwareProcessor initialization
"""
...
class EnhancedKeywordIndexerWorker:
    ...
    def __init__(self):
        ...
        # Initialize structure-aware processor with required parameters
        if self.opensearch_client:
            self.structure_processor = StructureAwareProcessor(self.opensearch_client, self.index_name)
        else:
            self.structure_processor = None
            logger.warning("OpenSearch client not available - structure processing disabled")
    ...
    def _create_opensearch_client(self):
        ...
        try:
            ...
            awsauth = AWSRequestsAuth(aws_access_key=credentials.access_key, aws_secret_access_key=credentials.secret_key, aws_token=credentials.token, aws_host=self.opensearch_endpoint.replace("https://", ""), aws_region=self.region, aws_service="aoss")
    ...
    def lambda_handler(self, event, context):
        ...
        try:
            ...
            metadata = {"doc_id": doc_id, "status": "processing"}
            ...
            if False and structure_data and self.structure_processor:
    ...
    def process_with_structure(self, doc_id: str, structure_data: Dict, metadata: Dict) -> Dict:
        ...
        try:
            # Extract full text from structure_data
            full_text = structure_data.get('full_text', '')
            textract_structure = structure_data
            
            # Use structure-aware processor (corrected method call - no filename param)
            doc_data = self.structure_processor.prepare_structure_enhanced_document(
                doc_id, full_text, textract_structure, metadata
            )
            
            # Build processing_result structure from doc_data
            processing_result = {
                'success': True,
                'keywords': doc_data.get('keywords', []),
                'keywords_count': len(doc_data.get('keywords', [])),
                'processing_method': 'structure_aware',
                'doc_data': doc_data
            }
            
            # Index in OpenSearch using the doc_data directly
            if self.opensearch_client:
                opensearch_result = self.index_in_opensearch_direct(doc_id, doc_data)
    ...
    def index_in_opensearch_direct(self, doc_id: str, doc_data: Dict) -> Dict:
        """Index pre-prepared doc_data in OpenSearch"""
        try:
            if not self.opensearch_client:
                return {'opensearch_indexed': False, 'opensearch_error': 'Client not available'}
            
            # doc_data is already prepared by structure processor
            safe_doc_data = self.sanitize_for_opensearch(doc_data)
