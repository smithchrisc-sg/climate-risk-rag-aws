...
class StandardizedMessagePublisher:
    ...
    def publish_vector_embeddings_ready(self,
                                      doc_id: str,
                                      doc_hash: str,
                                      chunks_location: str,
                                      text_location: str,
        ...
        message = {
            ...
            "data_locations": {
                "text_location": text_location,
                "chunks_location": chunks_location,
                "structure_location": text_location.replace("raw_text.txt", "textract_response.json") if text_location else None
            },
            "processing_metadata": {
                "chunks_count": processing_metadata.get("chunks_count", 0),
                "initiated_from": processing_metadata.get("initiated_from", "chunks_ready"),
                "processing_started": processing_metadata.get("processing_started"),
        ...
        try:
            response = self.sns.publish(
                TopicArn=topic_arn,
                Message=json.dumps(message, default=str),
                Subject=f"Vector embeddings ready for processing: {doc_id}"
            )
            
            logger.info(f"Published vector embeddings ready message for {doc_id}: {response['MessageId']}")
            
        except Exception as e:
            logger.error(f"Failed to publish vector embeddings ready message: {e}")
            raise
