...
class TextExtractorInitiator:
    ...
    def __init__(self):
        ...
        logger.info(f"Output bucket: {os.environ.get('OUTPUT_BUCKET', 'Not set')}")
    ...
    def store_job_metadata(self, job_id: str, doc_id: str, bucket: str, key: str):
        """Store Textract job metadata in database"""
        try:
            ...
            # Insert job metadata
            insert_query = """
                INSERT INTO textract_jobs (job_id, doc_id, s3_bucket, s3_key, status, created_at)
                VALUES (%s, %s, %s, %s, %s, %s)
            ...
            cursor.execute(insert_query, (
                job_id, doc_id, bucket, key, 'IN_PROGRESS', datetime.utcnow()
            ))
            
            # Update document processing status
            status_query = """
                INSERT INTO document_processing_status (doc_hash, text_extraction_status, updated_at)
                VALUES (%s, %s, %s)
            ...
            cursor.execute(status_query, (
                doc_id, 'IN_PROGRESS', datetime.utcnow()
            ))
