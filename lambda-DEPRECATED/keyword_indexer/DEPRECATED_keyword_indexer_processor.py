...
class KeywordIndexerProcessor:
    ...
    def process_text_ready_message(self, message: Dict) -> Dict:
        ...
        # Extract message components - handle both old and new message formats
        doc_id = message.get('doc_id')
        full_text_location = message.get('full_text_location') or message.get('data_locations', {}).get('text_location')
        filename = message.get('filename', message.get('document_metadata', {}).get('original_filename', 'unknown'))
    ...
    def read_full_text(self, location) -> str:
        """Read full text from S3 location (supports both dict and S3 URL string formats)"""
        
        try:
            # Handle both dictionary format and S3 URL string format
            if isinstance(location, dict):
                bucket = location['bucket']
                key = location['key']
            elif isinstance(location, str) and location.startswith('s3://'):
                # Parse S3 URL: s3://bucket/path/to/file.txt
                s3_parts = location[5:].split('/', 1)  # Remove 's3://' and split on first '/'
                bucket = s3_parts[0]
