...
class SimpleAsyncKeywordIndexer:
    ...
    def __init__(self):
        ...
        # Database manager - use layer import only
        try:
            from utils.DatabaseManager import DatabaseManager
            self.db_manager = DatabaseManager()
            logger.info("DatabaseManager initialized from layer")
        except ImportError as e:
            self.db_manager = None
            logger.warning(f"DatabaseManager not available - status updates disabled: {e}")
